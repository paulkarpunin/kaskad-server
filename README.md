...
Проект спроектирован с упором на системную безопасность, изоляцию состояний и производительность:

* **Детерминированное управление (Tagging):** Каждому туннелю присваивается уникальный криптографический идентификатор (`CASCADE_ID`) на уровне ядра через модуль `iptables comment`. Это исключает ложные срабатывания и конфликт с правилами других сервисов (Docker, Fail2Ban).
* **Атомарные транзакции:** Деструкция и модификация правил маршрутизации выполняется через связку `iptables-save | iptables-restore`. Это гарантирует мгновенное применение изменений (O(1) time complexity) и защиту от «гонки данных».
* **Оптимизация сетевого стека:** Скрипт автоматически применяет изолированные параметры ядра (Sysctl), активируя алгоритм контроля перегрузок **Google BBR** и дисциплину очереди `fq` для минимизации задержек (latency) и потерь пакетов.
* **Хирургическая интеграция с UFW:** Вместо глобального открытия политики `FORWARD`, система прописывает точечные разрешающие маршруты исключительно для портов активных туннелей.
* **Изоляция зависимостей:** Математическая валидация IPv4-адресов защищает от инъекций команд (Command Injection).
